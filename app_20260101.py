...
<style>
    ...
    /* 🔴 關鍵修正 2：針對滑鼠懸停 (Hover) 的效果 */
    .stButton > button:hover {
        background-color: #333333; /* 背景變深灰 */
...
# 首頁：商品列表
    ...
    # 商品 1：專題計畫書 (超安全，純排版)
    with col1:
        st.image("https://placehold.co/600x400/000000/ffffff?text=Proposal+Template", use_column_width=True)
        st.markdown("### 研究計畫書標準格式範本")
        st.markdown("""
        **格式**：Word (.docx)  
        **適用**：大專院校專題計畫、國科會計畫申請  
        
        依照最新學術計畫格式編排，包含摘要、研究動機、文獻回顧及預期成果之標準排版設定，解決繁瑣的縮排與引用格式問題。也提供撰寫Reference Li各種注意事項，讓您的計劃書專業度升級。
        """)
        st.markdown("#### NT$ 150")
        if st.button("立即購買", key="btn1"):
    ...
    # 商品 2：海報設計 (設計類，絕對安全)
    with col2:
        st.image("https://placehold.co/600x400/333333/ffffff?text=Poster+Pack", use_column_width=True)
